...
servercall="http://192.168.0.17:8080" 
...
class RTL_433_detect_class:#fills variable data with values from rtl_433
	...
	wdata=int(winddata)
	#self.data['WD']=
	if wdata>=349 and wdata <=11: self.data['WD']= 'N'
	elif wdata>11 and wdata <34: self.data['WD']= 'NNE'
	elif wdata>=34 and wdata <56: self.data['WD']= 'NE'
	elif wdata>=56 and wdata <79: self.data['WD']= 'ENE'
	elif wdata>=79 and wdata <101: self.data['WD']= 'E'
	elif wdata>=101 and wdata <124: self.data['WD']= 'ESE'
	elif wdata>=124 and wdata <146: self.data['WD']= 'SE'
	elif wdata>=146 and wdata <169: self.data['WD']= 'SSE'
	elif wdata>=169 and wdata <191: self.data['WD']= 'S'
	elif wdata>=191 and wdata <214: self.data['WD']= 'SSW'
	elif wdata>=214 and wdata <236: self.data['WD']= 'SW'
	elif wdata>=236 and wdata <259: self.data['WD']= 'WSW'
	elif wdata>=259 and wdata <281: self.data['WD']= 'W'
	elif wdata>=281 and wdata <304: self.data['WD']= 'WNW'
	elif wdata>=304 and wdata <326: self.data['WD']= 'NW'
	elif wdata>=326 and wdata <349: self.data['WD']= 'NNW'
	return
...
# TODO: read devices domotics and check if they all exist in config of this script
class RTL_433_Domoticz_class:
    def __init__(self, passservercall):
        self.hardwareid=9999
        self.servercall=passservercall
	...
	self.jsontablefilename=deamon_configfile
        ...
        if not self.check_existance_RTL_433_in_DOMOTICZ():
            ...
            if not self.check_existance_RTL_433_in_DOMOTICZ():#Check once more
                print "Halting application as hardware RTL_433 cannot be created. Please check server parameters"
...
#	249 TypeAirQuality    0	     nvalue=PPM        
        ...
        print 'Creating Device RTL_433 on Domotics server '+servercall
        request=self.servercall+"/json.htm?type=createvirtualsensor&idx="+self.hardwareid+"&sensortype="+str(typeid)
        ...
        try:
            ...
            if results.get(u'status')=='OK':
                ...
                if createid:
                    print 'Successful creating device',createid
		    self.devicelist[device_id]=str(createid)
		    self.write_jsontable()                    
                    return 1 #Result OK
                else: 
                    print 'ERROR creating device. skipping for now!'
                    return 0
        except:
            print "ERROR: Time out URL (create_device_domoticz)"
            pass
        print 'ERROR creating device. Skipping for now!'
        return 0 #Result NOK
    ...
    def get_latest_deviceid(self):#FALSE = does not exist TRUE = exists
        ...
        try:
            ...
            print "There was an error: Timout URL (get_latest_deviceid)"
            #return 0
            pass
        results = json.loads(search_results)
    ...
    def check_existance_RTL_433_in_DOMOTICZ(self):#FALSE = does not exist TRUE = exists
        ...
        try:
            ...
            print "There was an error: Timout URL"
            pass
            return 0
        search_results = search_response.read()
        results = json.loads(search_results)
        for item in results.get(u'result', []):
            if item.get(u'Name')=='RTL_433':
                ...
                return 1 #Does exist
        print 'No hardware RTL_433 found in Domoticz'	
        return 0 #Does not exist!

    def create_hardware_RTL_433(self):
        print 'Creating Hardware RTL_433 on Domotics server '+servercall
	...
	print request
        ...
        print 'ERROR creating hardware RTL_433'
        return 0 #Result NOK
    ...
    def push_data(self,data):
		...
		self.create_device_domoticz(data['Device_id'],domoticzdevice)
            try:domoticzid=self.devicelist[data['Device_id']]
            except:domoticzid=0#just set false
        if domoticzid:
            print "pushing data" #Watch that Battery =0-100 or 255 of no value
		...
		#2=Dry <25
		#3=Wet >60
		if data['Humidity']>60:data['Comfort']='3'
		elif data['Humidity']<25:data['Comfort']='2'
		elif data['Humidity']>45 and data['Humidity']<55:data['Comfort']='1'
		else: data['Humidity']='0'
		#/json.htm?type=command&param=udevice&idx=IDX&nvalue=0&svalue=TEMP;HUM;HUM_STAT
		request=self.servercall+"/json.htm?type=command&param=udevice&idx="+str(domoticzid)+"&nvalue=0&svalue="+data['Temperature']+";"+data['Humidity']+";"+data['Comfort']+"&battery="+data['Battery']
	    elif domoticzdevice=="80":
	       request=self.servercall+"/json.htm?type=command&param=udevice&idx="+str(domoticzid)+"&nvalue=0&svalue="+	data['Temperature']+"&battery="+data['Battery']
		...
		#http://192.168.99.212:8080/json.htm?type=command&param=udevice&idx=8&nvalue=45&svalue=45;0
                request=self.servercall+"/json.htm?type=command&param=udevice&idx="+str(domoticzid)+"&nvalue="+data['Humidity']+"&svalue="+data['Humidity']+";"+data['Comfort']+"&battery="+data['Battery'] 
	    elif domoticzdevice=="86":#WIND
		#/json.htm?type=command&param=udevice&idx=IDX&nvalue=0&svalue=WB;WD;WS;WG;22;24
		request=self.servercall+"/json.htm?type=command&param=udevice&idx="+str(domoticzid)+"&nvalue=0&svalue="+data['WB']+";"+data['WD']+";"+data['WS']+";"+data['WG']+";0;0&battery="+data['Battery']
		print request
	    
	    elif domoticzdevice=="85":	#Rain    
	       request=self.servercall+"/json.htm?type=command&param=udevice&idx="+str(domoticzid)+"&nvalue=0&svalue=0;"+data['Rain']+"&battery="+data['Battery']
